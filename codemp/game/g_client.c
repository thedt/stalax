...
//	char		*areabits;
	...
	//[Stalax]
	connectEvent = G_LuaCallClientConnectEvent(clientNum, firstTime, isBot, userinfo);
...
//	areabits = client->areabits;
	...
	if ( firstTime ) {
		...
		Q_strncpyz(savedIP, g_sess[client-level.clients].IPstring, sizeof(savedIP));
		G_InitSessionData( client, userinfo, isBot );
		Q_strncpyz(client->sess.IPstring, savedIP, sizeof(client->sess.IPstring));
		G_WriteClientSessionData( client );
	}
	G_ReadSessionData( client, firstTime, isBot );
...
void ClientBegin( int clientNum, qboolean allowTeamReset ) {
	...
	gentity_t	*tent;
	int			flags, i;
	char		userinfo[MAX_INFO_VALUE], *modelname, *value;
	qboolean	gotPlugin = qfalse;
	...
	value = Info_ValueForKey (userinfo, "stalax");
	if (Q_stricmp( value, GAMEVERSION ) && !(ent->r.svFlags & SVF_BOT))
	{
		if (sta_forcePlugin.integer > 0 || g_maxclients.integer > 32)
			trap_DropClient(clientNum, "Please download the latest version at \nhttp://bit.ly/stalax");
			//return "Please download the latest version at \nhttp://bit.ly/stalax";
	}
	else
		client->pers.hasPlugin = qtrue;
	...
	G_LuaCallClientBeginEvent(ent->client->ps.clientNum);
}
...
void ClientDisconnect( int clientNum ) {
	...
	G_LuaCallClientDisconnectEvent(ent->client->ps.clientNum);
