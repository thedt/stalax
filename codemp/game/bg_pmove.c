			...
			if ( anim != -1 )
			{
				...
				{//there is a wall there.. or hit a client
					...
					{//wall-runs can only run on perfectly flat walls, sorry. 
						...
						{
#ifdef QAGAME
							if (sta_flipkick.integer && trace.entityNum < MAX_CLIENTS)
#else
...
#ifdef QAGAME
			else if (sta_flipkick.integer && pm->cmd.forwardmove > 0 //pushing forward
#else
...
#ifdef QAGAME
							if ( sta_flipkick.integer && kick && traceEnt && (traceEnt->s.eType == ET_PLAYER || traceEnt->s.eType == ET_NPC) )
#else
